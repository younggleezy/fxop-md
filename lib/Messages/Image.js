...
class Image extends Base {
  ...
  async reply(text, options = {}) {
    return this.sendMessage(this.jid, text, { ...options, quoted: this });
  }
    ...
    switch (type.toLowerCase()) {
      ...
      case "audio":
        if (Buffer.isBuffer(data)) {
          ...
          return this.client.sendMessage(
            recipient,
            { [type]: { url: data }, ...mergedOptions },
            mergedOptions,
          );
        }
        break;
      case "template":
        const optional = await generateWAMessage(
          recipient,
          data,
          mergedOptions,
        );
      ...
      case "sticker":
        ...
        if (mime === "image/webp") {
          ...
          await this.client.sendMessage(
            recipient,
            { sticker: { url: buff }, ...mergedOptions },
            mergedOptions,
          );
        } else {
  ...
  async sendFromUrl(url, options = {}) {
    ...
    const sendOptions =
      type === "audio" ? { ...options, mimetype: "audio/mpeg" } : options;
    return this.client.sendMessage(
      this.jid,
      { [type === "application" ? "document" : type]: buff, ...sendOptions },
      sendOptions,
    );
  }
