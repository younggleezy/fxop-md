...
class AllMessage extends Base {
  ...
  async sendMessage(jid, content, options = {}, type = "text") {
    ...
    switch (type.toLowerCase()) {
      case "text":
        return this.client.sendMessage(recipient, { text: content, ...mergedOptions });
      case "image":
      case "photo":
      case "video":
      case "audio":
        ...
        if (Buffer.isBuffer(content)) {
          return this.client.sendMessage(recipient, { [mediaType]: content, ...mergedOptions });
        } else if (isUrl(content)) {
          return this.client.sendMessage(recipient, { [mediaType]: { url: content }, ...mergedOptions });
        }
        break;
      case "template":
        ...
        await this.client.relayMessage(recipient, message, { messageId: optional.key.id });
        break;
      case "interactive":
        const genMessage = createInteractiveMessage(content);
        await this.client.relayMessage(recipient, genMessage.message, { messageId: genMessage.key.id });
        break;
      case "sticker":
  ...
  async edit(text, options = {}) {
    await this.client.sendMessage(this.jid, { text, edit: this.key, ...options });
  }
