...
const { generateWAMessageFromContent, generateForwardMessageContent, generateWAMessage, proto, getContentType, downloadContentFromMessage } = require("baileys");
...
class Message extends Base {
 ...
 async send(content, options = {}) {
  ...
  try {
   switch (type.toLowerCase()) {
    case "text":
     return await this.client.sendMessage(jid, { text: content, ...mergedOptions });

    case "image":
    case "video":
    case "audio":
     const mediaContent = Buffer.isBuffer(content) ? content : { url: content };
     return await this.client.sendMessage(jid, { [type]: mediaContent, ...mergedOptions });
 ...
 async downloadContent(message, type) {
  const contentStream = await downloadContentFromMessage(message, type);
 ...
 async forward(jid, content, options = {}) {
  try {
   if (!content) throw new Error("No content to forward");
   ...
   switch (contentType) {
    case "conversation":
     formattedMessage = { text: content.text, contextInfo: options.contextInfo };
     break;
    case "videoMessage":
     formattedMessage = { video: await this.downloadContent(content, "video"), mimetype: content.mimetype, caption: content.caption, contextInfo: options.contextInfo };
     break;
    case "imageMessage":
     formattedMessage = { image: await this.downloadContent(content, "image"), mimetype: content.mimetype, caption: content.caption, contextInfo: options.contextInfo };
     break;
    default:
     throw new Error("Unsupported message type");
   }

   const waMessage = generateWAMessageFromContent(jid, formattedMessage, options);
