...
const { generateWAMessageFromContent, generateForwardMessageContent, downloadContentFromMessage, generateWAMessage, proto, getContentType } = require("baileys");
...
class Message extends Base {
 ...
 async edit(messageKey, newMessage) {
  const { jid, client } = this;
  if (!messageKey) {
   throw new Error("Message key is required to edit the message.");
  }
  return await client.relayMessage(
   jid,
   {
    protocolMessage: {
     key: messageKey, // Use the provided message key
     type: 14, // Type 14 is for edited messages
     editedMessage: {
