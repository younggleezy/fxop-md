...
async function askAi(aiType, query) {
 ...
 try {
  switch (aiType) {
   ...
   case "thinkany":
    const { data: thinkanyResponse } = await axios.get(`${BASE_URL}${apiPaths.thinkany}?query=${query}`, { headers: { ApiKey: API_KEY } });
   ...
   case "lepton":
    const { data: leptonResponse } = await axios.get(`${BASE_URL}${apiPaths.lepton}?query=${query}`, { headers: { ApiKey: API_KEY } });
    return leptonResponse;

   case "gpt":
    const { data: gptResponse } = await axios.get(`${BASE_URL}${apiPaths.gpt}?prompt=${query}`, { headers: { ApiKey: API_KEY } });
    return gptResponse;

   case "blackbox":
    const { data: blackboxResponse } = await axios.get(`${BASE_URL}${apiPaths.blackbox}?query=${query}`, { headers: { ApiKey: API_KEY } });
    return blackboxResponse;

   case "chatgpt":
    const { data: chatgptResponse } = await axios.get(`${BASE_URL}${apiPaths.chatgpt}?prompt=${query}`, { headers: { ApiKey: API_KEY } });
   ...
   case "upscale":
    ...
    formData.append("image", fs.createReadStream(query));
