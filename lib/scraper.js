...
const BASE_API = `https://astro-api-v1-7f77aeb3cfdd.herokuapp.com`;
...
class Scraper {
 ...
 async spotify(query) {
  const spoturl = `/download/spotify?url=`;
  const apiResponse = await axios.get(BASE_API + spoturl + query);
  const downloadUrl = apiResponse.data.data.link;
  if (!downloadUrl) throw new Error("Download URL not found");
  const response = await axios.get(downloadUrl, { responseType: "arraybuffer" });
  return Buffer.from(response.data);
 }
 ...
 async blackbox(query) {
  const bburl = `/ai/blackbox?q=`;
  const apiResponse = await axios.get(BASE_API + bburl + query);
  if (apiResponse.data.success && apiResponse.data.result) {
   return apiResponse.data.result;
  } else {
   throw new Error("API ERROR");
...
//  const data = await downloader.spotify(query);
//  console.log(data);
// }
// test("https://open.spotify.com/track/1ko2lVN0vKGUl9zrU0qSlT?si=becbc527a2d64c25");
