...
async function m3u82Mp4(m3u8Url) {
 return new Promise((resolve, reject) => {
  ...
  const ffmpegProcess = cp.exec(command, (error, stdout, stderr) => {
   if (error) {
...
function ffmpeg(buffer, args = [], ext = "", ext2 = "") {
 return new Promise(async (resolve, reject) => {
  try {
   ...
   const ffmpegProcess = cp.spawn("ffmpeg", ["-y", "-i", tmp, ...args, out])
    .on("error", reject)
    .on("close", async code => {
     try {
...
function IronMan(x) {
 return `https://ironman.koyeb.app/${x}`;
}
...
module.exports = {
 ...
 AddMp3Meta: async (songbuffer, coverBuffer, options = { title: "fxop Whatsapp bot", artist: ["Xasena"] }) => {
  ...
  writer.setFrame("TIT2", options.title).setFrame("TPE1", ["fxop"]).setFrame("APIC", {
   type: 3,
   data: coverBuffer,
   description: "Xasena",
  });
