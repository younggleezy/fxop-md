...
const { jidDecode, delay, generateWAMessageFromContent, proto } = require('astrofx011')
...
/**
 * Converts an M3U8 stream to an MP4 file and returns the data as a buffer.
 *
 * @param {string} m3u8Url - The URL of the M3U8 stream to be converted.
 * @returns {Promise<Buffer>} - A promise that resolves with the MP4 file data as a buffer.
 */
async function convertM3U8ToMp4(m3u8Url) {
 return new Promise((resolve, reject) => {
  const writableBuffer = new streamBuffers.WritableStreamBuffer({
   initialSize: 100 * 1024, // Initial buffer size of 100KB
   incrementAmount: 10 * 1024, // Increment by 10KB
  })

  const outputFilePath = 'output.mp4'
  const ffmpegCommand = `"${ffmpegPath}" -i "${m3u8Url}" -c copy "${outputFilePath}"`
  ...
  const ffmpegProcess = exec(ffmpegCommand, (error, stdout, stderr) => {
   ...
   fs.readFile(outputFilePath, (err, data) => {
    ...
    resolve(writableBuffer.getContents())
   })
  })

  ffmpegProcess.stderr.on('data', data => {
   const progressOutput = data.toString()
   const timeMatch = progressOutput.match(/time=(\d{2}:\d{2}:\d{2}\.\d{2})/)
   if (timeMatch) {
...
module.exports = {
 ...
 parsedJid(text = '') {
  const numbers = [...text.matchAll(/([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
  const mentions = [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
  const groups = [...text.matchAll(/([0-9]{18,24})/g)].map(v => v[1] + '@g.us')

  return [...new Set([...numbers, ...mentions, ...groups])]
 },
 getJson,
 isIgUrl: url => {
