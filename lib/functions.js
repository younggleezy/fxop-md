...
async function editAudio(message, effect = "bass") {
 try {
  ...
  // Download the audio as a buffer
  let downloadedAudioBuffer = await message.client.download(message.quoted);

  // Create a readable stream from the buffer
  const inputStream = Readable.from(downloadedAudioBuffer);
  let outputBuffers = [];

  // Create ffmpeg command and apply the audio filter
  ffmpeg(inputStream)
   ...
   .on("end", async () => {
    const processedAudio = Buffer.concat(outputBuffers);
