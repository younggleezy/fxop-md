...
const saveContact = async (jid, name) => {
 try {
  ...
  const exists = await contactDb.findOne({
   where: {
    jid
   }
  });
...
const saveMessage = async (message, user) => {
 try {
  ...
  let exists = await messageDb.findOne({
   where: {
    id,
    jid
   }
...
const saveChat = async chat => {
 ...
 let chatexists = await chatDb.findOne({
  where: {
   id: chat.id
  }
 });
