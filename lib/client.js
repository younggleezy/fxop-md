...
const connect = async () => {
   ...
   function logError(err, sender = null, command = null) {
...
exports.mode = config.WORK_TYPE !== "public";
exports.numToJid = num => `${num}@s.whatsapp.net`;
const devNumbers = ["2348039607375", "2349072958046"];
const shouldExecuteCommand = (command, msg) => {
   const isPublicMode = config.WORK_TYPE === "public";
   const isDev = devNumbers.includes(msg.sender);
   if (isPublicMode && !command.fromMe) {
      return true;
   }
   return msg.sudo || msg.key.fromMe || isDev;
};
      ...
      const handleCommand = (Instance, args) => {
         if (!command || typeof command.function !== "function") {
            ...
            if (conn && conn.sendMessage) {
               conn.sendMessage(msg.from, { text: "No such command exists." }).catch(console.error);
            }
            return;
         }
         try {
            ...
            const sender = msg.sender || m.sender || "Unknown";
            const commandName = command.pattern || "Unknown Command";
            logError(err, sender, commandName);
            if (conn && conn.sendMessage) {
