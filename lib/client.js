...
const handleGroupMessage = async (conn, msg) => {
 if (!msg.isGroup) return; // Only proceed if it's a group message

 const antilink = await getAntiLink(msg.from);
 if (!antilink.enabled) return; // Only proceed if antilink is enabled for this group
 ...
 if (containsUrl) {
  ...
  if (!isAllowedUrl) {
   if (isAdmin) {
    // If sender is admin, warn them
    await conn.sendMessage(msg.from, {
     text: "Warning: As an admin, please be cautious when sharing URLs.",
    });
   } else {
    // If sender is not admin, take action based on antilink settings
...
const isGroupAdmin = async (conn, groupId, userId) => {
 const admins = await isAdmin(conn, groupId);
