...
const emojis = require("./reactions"); // Load emojis
...
const handleGroupMessage = async (conn, msg) => {
   ...
   if (config.ANTIWORD && msg.body) {
      try {
         ...
         console.error("Error in anti-word feature:", error);
      }
   }
};
   ...
   async ({ messages, type }) => {
      if (type === "notify") {
         for (const msg of messages) {
            const serializedMsg = await serialize(JSON.parse(JSON.stringify(msg)), conn);
            if (!serializedMsg) continue;

            await saveMessage(msg, serializedMsg.sender);
            if (config.AUTO_READ) await conn.readMessages([serializedMsg.key]);
            if (config.AUTO_STATUS_READ && serializedMsg.from === "status@broadcast") await conn.readMessages([serializedMsg.key]);

            const pausedChats = await PausedChats.getPausedChats();
            if (pausedChats.some(chat => chat.chatId === serializedMsg.from) && !new RegExp(`${config.HANDLERS}( ?resume)`, "is").test(serializedMsg.body)) continue;

            if (serializedMsg.isGroup) await handleGroupMessage(conn, serializedMsg);

            await handlePlugins(conn, serializedMsg);

            // Handle auto-react and logs
            if (config.AUTO_REACT && serializedMsg.type === "chat") {
               if (emojis.length > 0) {
                  const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
                  await conn.sendMessage(serializedMsg.from, { react: { text: randomEmoji, key: serializedMsg.key } });
               }
            }

            if (config.LOGS && serializedMsg) {
               const name = await getName(serializedMsg.sender);
               const isGroup = serializedMsg.from.endsWith("@g.us");
               const messageContent = serializedMsg.body || serializedMsg.type;

               if (messageContent !== undefined) {
                  if (isGroup) {
                     const groupName = (await conn.groupMetadata(serializedMsg.from)).subject;
...
const handlePlugins = async (conn, msg) => {
   for (const command of plugins.commands) {
      if (!shouldExecuteCommand(command, msg)) continue;
      ...
      if (msg.body && command.pattern) {
         ...
         if (match) {
            ...
            await executeCommand(command, conn, msg, [match[3] || false]);
         }
      } else if (command.on === "message" || (msg.type && command.on === msg.type)) {
         await executeCommand(command, conn, msg, [msg.body]);
      }
   }
};

const executeCommand = async (command, conn, msg, args) => {
   if (typeof command.function !== "function") return;
   try {
      const Instance = command.on === "message" ? require("./class").AllMessage : require("./class")[msg.type];
      if (Instance) {
