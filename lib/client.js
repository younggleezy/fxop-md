...
const updateStatus = async (conn) => {
  let currentIndex = 0;
  setInterval(async () => {
    if (config.AUTO_STATUS && config.STATUS_MESSAGES.length > 0) {
      await conn.updateProfileStatus(config.STATUS_MESSAGES[currentIndex]);
      currentIndex = (currentIndex + 1) % config.STATUS_MESSAGES.length;
    }
  }, 3600000); // Update every hour
};

// New feature: Command cooldown
const cooldowns = new Map();

const applyCooldown = (userId, commandName, cooldownTime) => {
