...
const handleContactsUpdate = update => {
   logger.info("Contacts update", { update });

   update.forEach(contact => {
      const { id, notify, verifiedName, name, imgUrl } = contact;

      const contactUpdate = { id };
      if (notify !== undefined) contactUpdate.notify = notify;
      if (verifiedName !== undefined) contactUpdate.verifiedName = verifiedName;
      if (name !== undefined) contactUpdate.name = name;
      if (imgUrl !== undefined) contactUpdate.imgUrl = imgUrl;

      logger.info("Contact Update", contactUpdate);
   });
};

const handleGroupsUpdate = update => {
   logger.info("Groups update", { update });

   update.forEach(groupUpdate => {
      const { id, subject, size, desc, restrict, announce, owner } = groupUpdate;

      const groupUpdateInfo = { id };
      if (subject !== undefined) groupUpdateInfo.subject = subject;
      if (size !== undefined) groupUpdateInfo.size = size;
      if (desc !== undefined) groupUpdateInfo.desc = desc;
      if (restrict !== undefined) groupUpdateInfo.restrict = restrict;
      if (announce !== undefined) groupUpdateInfo.announce = announce;
      if (owner !== undefined) groupUpdateInfo.owner = owner;

      logger.info("Group Update", groupUpdateInfo);
   });
};

const handleMessageReceipt = receipt => {
   logger.info("Message receipt", { receipt });

   receipt.forEach(msg => {
      const { key, receipt: receiptType, receiptTimestamp } = msg;

      const receiptInfo = {
         id: key.id,
         from: key.remoteJid,
         to: key.fromMe ? "Me" : key.participant,
         receipt: receiptType,
         timestamp: new Date(receiptTimestamp * 1000).toLocaleString(),
      };

      if (receiptType === "read") {
         receiptInfo.status = "Message has been read";
      } else if (receiptType === "played") {
         receiptInfo.status = "Audio/Video message has been played";
      }

      logger.info("Message Receipt", receiptInfo);
   });
};
