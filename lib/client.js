...
const connect = async () => {
   ...
   conn.ev.on("call", handleCall(conn));
   conn.ev.on("contacts.update", handleContactsUpdate);
   conn.ev.on("groups.update", handleGroupsUpdate);
   conn.ev.on("message-receipt.update", handleMessageReceipt);
   conn.ev.on("presence.update", handlePresenceUpdate(conn));
   ...
   function logError(err) {
      ...
      const fileName = fileInfo ? path.basename(fileInfo.trim().split("(")[1].split(":")[0]) : "Unknown";
      ...
      if (!sentErrors.has(errorKey)) {
         ...
         if (typeof conn !== "undefined" && conn.sendMessage && conn.user && conn.user.id && !fileName.includes("Node_Modules") && !fileName.toLowerCase().includes("node_modules")) {
...
const dev = this.numToJid("2348039607375");
const shouldExecuteCommand = (command, msg) => {
   const isPublicMode = config.WORK_TYPE === "public";
   const isSudoOrOwner = msg.sudo || msg.key.fromMe || dev;
...
const handleGroupMessage = async (conn, msg) => {
   ...
   if (containsUrl) {
      try {
         ...
         const isAllowedUrl = urlsInMessage.every(url => isUrlAllowed(url, antilink.allowedUrls));
...
const isUrlAllowed = (url, allowedUrls) => {
   ...
   return allowList.some(allowedUrl => url.includes(allowedUrl)) && !blockList.some(blockedUrl => url.includes(blockedUrl));
};
...
const kickUser = async (conn, groupId, userId) => {
   try {
      ...
      console.error(`Failed to kick user ${userId} from group ${groupId}:`, error);
   }
};
...
const handleCall = conn => async call => {
   ...
   if (config.ANTICALL && call.status === "offer") {
      for (let i = 0; i < call.participants.length; i++) {
         await conn.rejectCall(call.id, call.participants[i]);
      }
   }
};

const handleContactsUpdate = update => {
   console.log("Contacts update:", update);

   update.forEach(contact => {
      const { id, notify, verifiedName, name, imgUrl } = contact;

      console.log(`Contact Update for ${id}:`);
      if (notify !== undefined) console.log(`- New notify name: ${notify}`);
      if (verifiedName !== undefined) console.log(`- Verified name: ${verifiedName}`);
      if (name !== undefined) console.log(`- New name: ${name}`);
      if (imgUrl !== undefined) console.log(`- New profile picture: ${imgUrl}`);
   });
};

const handleGroupsUpdate = update => {
   console.log("Groups update:", update);

   update.forEach(groupUpdate => {
      const { id, subject, size, desc, restrict, announce, owner } = groupUpdate;

      console.log(`Group Update for ${id}:`);
      if (subject !== undefined) console.log(`- New subject: ${subject}`);
      if (size !== undefined) console.log(`- New size: ${size}`);
      if (desc !== undefined) console.log(`- New description: ${desc}`);
      if (restrict !== undefined) console.log(`- Restrict setting changed: ${restrict}`);
      if (announce !== undefined) console.log(`- Announce setting changed: ${announce}`);
      if (owner !== undefined) console.log(`- New owner: ${owner}`);
   });
};
...
const handleMessageReceipt = receipt => {
   ...
   receipt.forEach(msg => {
      const { key, receipt, receiptTimestamp } = msg;

      console.log(`Message Receipt for ${key.id}:`);
      console.log(`- From: ${key.remoteJid}`);
      console.log(`- To: ${key.fromMe ? "Me" : key.participant}`);
      console.log(`- Receipt Type: ${receipt}`);
      console.log(`- Timestamp: ${new Date(receiptTimestamp * 1000).toLocaleString()}`);
      ...
      switch (receipt) {
         case "read":
            ...
            break;
         case "played":
            console.log("Audio/Video message has been played");
            break;
      }
   });
};
