...
class AllMessage extends Base {
  ...
  async send(content, options = {}) {
    ...
    switch (type.toLowerCase()) {
      ...
      case "video":
      case "audio":
        const mediaContent = Buffer.isBuffer(content)
          ? content
          : { url: content };
        return this.client.sendMessage(jid, {
          [type]: mediaContent,
  ...
  async sendMessage(
    jid,
    content,
    opt = { packname: "ᴀsᴛʀᴏ", author: "ғxᴏᴘ-ᴍᴅ" },
    type = "text",
  ) {
    switch (type.toLowerCase()) {
      case "text":
