...
command(
 ...
 async (message, match, m) => {
  ...
  const antiword = await parseDB(message.jid, "antiword");
...
command(
 ...
 async (message, match) => {
  ...
  const { antifake } = await fetchDB(message.chat);
  if (match.toLowerCase() === "get") {
   ...
   await createDB(message.jid, data, "true", "antifake");
   return await message.send(`_Antifake Activated_`);
  } else if (match.toLowerCase() === "off") {
   const data = antifake && antifake.message ? antifake.message : "";
   await createDB(message.jid, data, "false", "antifake");
  ...
  await createDB(message.jid, match, status, "antifake");
 ...
 async (message, match) => {
  ...
  if (!match)
   return await message.send(
    "Choose settings to change antidemote settings",
    {
     values: [
      { displayText: "on", id: "antidemote on" },
      { displayText: "off", id: "antidemote off" },
     ],
     onlyOnce: true,
     withPrefix: true,
...
command(
 ...
 async (message, match) => {
  ...
  if (!match)
   return await message.send(
    "Choose settings to change antipromote settings",
    {
     values: [
      { displayText: "on", id: "antipromote on" },
      { displayText: "off", id: "antipromote off" },
     ],
     onlyOnce: true,
     withPrefix: true,
     participates: [message.sender],
...
command(
 ...
 async (message, match) => {
  ...
  const antidelete = await parseDB(message.user.id, "antidelete");
