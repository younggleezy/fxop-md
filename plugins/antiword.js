...
command(
  ...
  async (message, match) => {
    ...
    switch (action.toLowerCase()) {
      ...
      case "remove":
        if (!word)
          return await message.reply("_Please provide a word to remove_");
      ...
      case "list":
        ...
        if (antiWords.length === 0)
          return await message.reply("_No words in antiword list_");
        return await message.reply(`_Antiwords: ${antiWords.join(", ")}_`);

      default:
        return await message.reply(
          "_Invalid action. Use 'add', 'remove', or 'list'_",
        );
    }
  },
);
