...
command(
 ...
 async (message, match, m) => {
  if (!match || !message.reply_message) return await message.sendReply("_Provide An Image Prompt!_");
  await message.sendReply("_Generating Image!_");
  const generatedImg = await askAi("dalle", match || message.reply_message);
  const dalleMsg = `*_Generated By FXOP_*`;
  return await message.send(generatedImg, {
   caption: dalleMsg,
   contextInfo: {
    forwardingScore: 1,
    isForwarded: true,
