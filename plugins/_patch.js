...
command(
  ...
  async (message, match) => {
    if (match) {
      const matchedCommands = plugins.commands.filter(
        (i) =>
          i.pattern instanceof RegExp && i.pattern.test(message.prefix + match),
      );
      matchedCommands.forEach((i) => {
        const cmdName = i.pattern.toString().split(/\W+/)[1];
        message.reply(
          `\`\`\`Command: ${message.prefix}${cmdName.trim()}\nDescription: ${i.desc}\`\`\``,
        );
      });
      return;
    }
    ...
    const [date, time] = new Date()
      .toLocaleString("en-IN", { timeZone: TIME_ZONE })
      .split(",");
