...
bot(
 ...
 async message => {
  ...
  if (!isban) return
  await message.sendReply('_Antibot Detected A Baileys Bot!_')
  await message.sendReply('kicking!')
...
bot(
 ...
 async (message, match) => {
  ...
  if (!match) return await message.sendReply('Provide vaild action, on | off')
...
bot(
 ...
 async (message, match) => {
  ...
  if (!match) {
   ...
   return await message.sendReply(replyMsg)
  }

  if (match === 'info') {
   let msg = await getMessage(message.jid, 'welcome')
   if (!msg) return await message.sendReply('_There is no welcome set_')
   return message.sendReply(msg.message)
  }

  if (match === 'on') {
   let msg = await getMessage(message.jid, 'welcome')
   if (!msg) return await message.sendReply('_There is no welcome message to enable_')
   if (status) return await message.sendReply('_Welcome already enabled_')
   await toggleStatus(message.jid)
   return await message.sendReply('_Welcome enabled_')
  }

  if (match === 'off') {
   if (!status) return await message.sendReply('_Welcome already disabled_')
   await toggleStatus(message.jid, 'welcome')
   return await message.sendReply('_Welcome disabled_')
  }

  if (match == 'delete') {
   await delMessage(message.jid, 'welcome')
   return await message.sendReply('_Welcome message deleted_')
  }
  await setMessage(message.jid, 'welcome', match)
  return await message.sendReply('_Welcome message set_')
 }
)
...
bot(
 ...
 async (message, match) => {
  ...
  if (match === 'info') {
   ...
   if (!msg) return await message.sendReply('_There is no goodbye set_')
   return message.sendReply(msg.message)
  }

  if (match === 'on') {
   await toggleStatus(message.jid, 'goodbye')
   return await message.sendReply('_Goodbye enabled_')
  }

  if (match === 'off') {
   await toggleStatus(message.jid)
   return await message.sendReply('_Goodbye disabled_')
  }

  if (match === 'delete') {
   await delMessage(message.jid, 'goodbye')
   return await message.sendReply('_Goodbye deleted_')
  }

  await setMessage(message.jid, 'goodbye', match)
  return await message.sendReply('_Goodbye message set_')
 }
)
...
bot(
 ...
 async (message, match) => {
  ...
  if (!userId) return message.sendReply('_Mention or reply to someone_')
...
bot(
 ...
 async message => {
  ...
  return await message.sendReply(`_Warnings for @${userId.split('@')[0]} reset_`, {
   mentions: [userId],
  })
 }
)
...
bot(
 ...
 async (message, match) => {
  ...
  if (!reason) return message.sendReply('_Provide the reason to delete_')

  const warnInfo = await delWarn(userId, reason)
  if (warnInfo) {
   await message.sendReply(`_Warning reason deleted for @${userId.split('@')[0]}._ \n_Warn Count: ${warnInfo.warnCount}._`, { mentions: [userId] })
  } else {
   await message.sendReply(`_No such warning reason found for @${userId.split('@')[0]}._`, { mentions: [userId] })
  }
  return
 }
)
...
bot(
 ...
 async (message, match) => {
  if (!message.isGroup) return await message.sendReply('_This command is for groups_')

  match = match || message.reply_message.jid
  if (!match) return await message.sendReply('_Mention user to add')
  ...
  return await message.sendReply(`_@${jid[0].split('@')[0]} added_`, {
   mentions: [jid],
  })
 }
)
...
bot(
 ...
 async (message, match) => {
  ...
  match = match || message.reply_message.jid
  if (!match) return await message.sendReply('_Mention user to kick_')
  ...
  return await message.sendReply(`_@${jid[0].split('@')[0]} kicked_`, {
   mentions: [jid],
  })
 }
)
bot(
 ...
 async (message, match) => {
  ...
  match = match || message.reply_message.jid
  if (!match) return await message.sendReply('_Mention user to promote_')
  ...
  return await message.sendReply(`_@${jid[0].split('@')[0]} promoted as admin_`, {
   mentions: [jid],
  })
 }
)
bot(
 ...
 async (message, match) => {
  ...
  match = match || message.reply_message.jid
  if (!match) return await message.sendReply('_Mention user to demote_')
  ...
  return await message.sendReply(`_@${jid[0].split('@')[0]} demoted from admin_`, {
   mentions: [jid],
  })
 }
)
...
bot(
 ...
 async (message, match, m, client) => {
  ...
  await message.sendReply('_Muting_')
...
bot(
 ...
 async (message, match, m, client) => {
  if (!message.isGroup) return await message.sendReply('_This command is for groups_')
  if (!isAdmin(message.jid, message.user, message.client)) return await message.sendReply("_I'm not admin_")
  await message.sendReply('_Unmuting_')
...
bot(
 ...
 async (message, match) => {
  ...
  if (!match) return message.sendReply('_Enter or reply to a text to tag_')
  if (!message.isGroup) return
  const { participants } = await message.client.groupMetadata(message.jid)
