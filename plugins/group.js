...
const {
  ...
  setAntiDemote,
  getAntiDemote,
} = require("../lib/database/groupSettings.js");
const {
  getAntiWords,
  addStrike,
  resetStrikes,
  getStrikes,
} = require("../lib/database/antiword");
command(
  {
    pattern: "add",
    fromMe: mode,
...
command(
  ...
  async (message, match) => {
    ...
    if (!message.reply_message || !message.reply_message.image)
      return await message.reply("_Reply to an image to set as group picture_");
...
command(
  ...
  async (message, match) => {
    ...
    if (requests.length === 0)
      return await message.reply("_No pending join requests_");
...
command(
  ...
  async (message, match) => {
    ...
    if (!match)
      return await message.reply(
        "_Provide the number(s) of the request(s) to accept, separated by commas_",
      );

    const indexes = match.split(",").map((num) => parseInt(num.trim()) - 1);
    const validIndexes = indexes.filter(
      (index) => index >= 0 && index < requests.length,
    );

    if (validIndexes.length === 0)
      return await message.reply("_Invalid request number(s)_");
    ...
    return await message.reply(
      `_Accepted ${validIndexes.length} join request(s)_`,
    );
  },
);
...
command(
  ...
  async (message, match) => {
    ...
    if (!match)
      return await message.reply(
        "_Provide the number(s) of the request(s) to reject, separated by commas_",
      );
    ...
    return await message.reply(
      `_Rejected ${validIndexes.length} join request(s)_`,
    );
  },
);
...
command(
  ...
  async (message, match) => {
    ...
    if (!match)
      return await message.reply(
        "_Provide a setting to change: 'announcement' or 'locked' or 'unlocked'_",
      );
    ...
    if (match === "announcement") setting = "announcement";
      ...
      return await message.reply(
        "_Invalid setting. Use 'announcement', 'locked', or 'unlocked'_",
      );
...
command(
  ...
  async (message, match) => {
    ...
    if (!question || options.length < 2)
      return await message.reply(
        "_Usage: .poll Question | Option1 | Option2 | ..._",
      );
...
command(
  ...
  async (message, match) => {
    ...
    if (!isAdmin(message.jid, message.user, message.client))
      return await message.reply("_I'm not admin_");
    await message.reply(
      "_This action will remove all non-admin participants from the group. Are you sure? Reply with 'yes' to confirm._",
    );

    const confirmation = await message.client.waitForMessage(
      message.jid,
      message.sender,
      30000,
    );
    ...
    return await message.reply(
      `_Successfully removed ${toRemove.length} participants._`,
    );
  },
);
...
command(
  ...
  async (message, match) => {
    ...
    return await message.reply(
      `_Anti-promote has been ${newStatus ? "enabled" : "disabled"} for this group._`,
    );
  },
);
...
command(
  ...
  async (message, match) => {
    ...
    return await message.reply(
      `_Anti-demote has been ${newStatus ? "enabled" : "disabled"} for this group._`,
    );
  },
);
    ...
    if (message.update === "promote" || message.update === "demote") {
      ...
      if (message.update === "promote") {
        ...
        if (antiPromoteEnabled) {
          ...
          const botIsAdmin = await isAdmin(
            groupJid,
            botClient.user.id.split(":")[0] + "@s.whatsapp.net",
            botClient,
          );

          if (botIsAdmin) {
            for (let participant of participants) {
              await botClient.groupParticipantsUpdate(
                groupJid,
                [participant],
                "demote",
              );
            ...
            await message.reply(
              `_Unauthorized promotion detected. User(s) have been demoted._`,
            );
          }
        }
        ...
        if (antiDemoteEnabled) {
          ...
          if (botIsAdmin) {
            ...
            await message.reply(
              `_Unauthorized demotion detected. User(s) have been re-promoted._`,
            );
          }
        }
      }
...
command(
  ...
  async (message, match) => {
    ...
    const userId =
      mentionedJid ||
      (message.reply_message ? message.reply_message.sender : null);

    if (!userId)
      return await message.reply(
        "_Please mention a user or reply to their message_",
      );
    ...
    switch (action.toLowerCase()) {
      case "check":
        ...
        return await message.reply(
          `_User @${userId.split("@")[0]} has ${strikes} strike(s)._`,
        );

      case "reset":
        await resetStrikes(message.jid, userId);
        return await message.reply(
          `_Strikes reset for user @${userId.split("@")[0]}._`,
        );
...
command(
  ...
  async (message) => {
    ...
    const containsAntiWord = antiWords.some((word) =>
    ...
    if (containsAntiWord) {
      ...
      if (strikes < MAX_STRIKES) {
        await message.reply(
          `_Warning: Your message was deleted because it contained a prohibited word. You have ${strikes} strike(s). At ${MAX_STRIKES} strikes, you will be removed from the group._`,
        );
      } else {
        try {
          await message.client.groupParticipantsUpdate(
          ...
          await message.reply(
            `_User @${message.sender.split("@")[0]} has been removed from the group for repeated violations._`,
          );
        } catch (error) {
          console.error("Failed to remove user:", error);
          await message.reply(
            "_Failed to remove user. Please ensure the bot has admin privileges._",
          );
        }
