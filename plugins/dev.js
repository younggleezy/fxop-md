 ...
 async (message, m, match, client) => {
  ...
  try {
   ...
   if (evalText.startsWith("cmd:")) {
    ...
    const cmdFunction = new Function("axios", "exec", "util", "message", "variableStore", cmdBody.join(" "));
   ...
   const asyncEval = new Function("axios", "exec", "util", "message", "variableStore", `return (async () => { ${evalText} })();`);
